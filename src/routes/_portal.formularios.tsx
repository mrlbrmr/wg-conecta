...
function FormulariosPage() {
  ...
  const categories = useMemo(
    () => Array.from(new Set(items.map((f) => f.category).filter(Boolean) as string[])),
    [items],
  );
  ...
  return (
    <>
      <PageHeader
        eyebrow="Portal WG"
        title="Formulários"
        description="Links úteis para solicitações do dia a dia."
      />
      {categories.length > 0 && (
        <div className="mb-4 flex flex-wrap gap-2">
          <button
            onClick={() => setCategory("")}
            className={`chip ${!category ? "!bg-primary !text-primary-foreground" : ""}`}
          >
            Todos
          </button>
          {categories.map((c) => (
            <button
              ...
              className={`chip ${category === c ? "!bg-primary !text-primary-foreground" : ""}`}
            >
              {c}
            </button>
          ))}
      ...
      {filtered.length === 0 ? (
        <EmptyState title="Nenhum formulário nesta categoria." />
      ) : (
          ...
          {filtered.map((f) => (
            ...
            >
              <div className="grid h-11 w-11 place-items-center rounded-xl bg-primary-softer text-primary group-hover:bg-primary group-hover:text-primary-foreground transition">
                {(() => {
                  const I = ICON_MAP[f.icon ?? ""] ?? ClipboardList;
              ...
              <div className="mt-auto inline-flex items-center gap-1 text-xs font-bold text-primary">
