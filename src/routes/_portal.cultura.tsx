...
function SectionTitle({
  ...
  return (
    <div className="flex items-center gap-2.5 mb-4">
      <div className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-primary-softer text-primary">
...
function CulturaPage() {
  ...
  return (
    <>
      <PageHeader
        eyebrow="Cultura WG"
        title="Nossa gente, nossa cultura"
        description="Aniversariantes, tempo de casa, reconhecimentos e campanhas."
      />
      ...
      <section className="mb-8">
        ...
        {bdMonth.length === 0 ? (
          <p className="text-sm text-muted-foreground card-soft p-6 text-center">
            ...
            {bdMonth.map((b) => (
              <div key={b.id} className="card-soft p-4 flex items-center gap-3">
                {b.photo_url ? (
                  ...
                  <div className="grid h-12 w-12 place-items-center rounded-full bg-accent-soft text-accent-foreground">
        ...
        {(bd.data ?? []).length > bdMonth.length && (
          <details className="mt-4 card-soft p-4">
            <summary className="cursor-pointer text-sm font-bold">
              Ver todos os aniversariantes do ano ({(bd.data ?? []).length})
            </summary>
            <div className="mt-4 grid gap-3 sm:grid-cols-2 md:grid-cols-3">
              {(bd.data ?? [])
                .filter((b) => b.birthday_month !== currentMonth)
                .map((b) => (
                  <div key={b.id} className="flex items-center gap-3">
                    {b.photo_url ? (
                      ...
                      <div className="grid h-10 w-10 place-items-center rounded-full bg-accent-soft text-accent-foreground">
      ...
      <section className="mb-8">
        ...
        {waMonth.length === 0 ? (
          <p className="text-sm text-muted-foreground card-soft p-6 text-center">
            Nenhum aniversário de casa neste mês.
          </p>
        ) : (
          <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
            {waMonth.map((w) => {
              ...
              return (
                <div key={w.id} className="card-soft p-4 flex items-center gap-3">
                  {w.photo_url ? (
                    <img
                      src={fileUrl(w.photo_url) ?? ""}
                      alt=""
                      className="h-12 w-12 rounded-full object-cover"
                    />
                  ) : (
                    <div className="grid h-12 w-12 place-items-center rounded-full bg-primary-softer text-primary">
        ...
        {(() => {
          ...
          return (
            <details className="mt-4 card-soft p-4">
              <summary className="cursor-pointer text-sm font-bold">
                Ver todos os aniversários de casa ({all.length})
              </summary>
              <div className="mt-4 grid gap-3 sm:grid-cols-2 md:grid-cols-3">
                {all
                    ...
                    return (
                      <div key={w.id} className="flex items-center gap-3">
                        {w.photo_url ? (
                          ...
                          <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-primary-softer text-primary">
