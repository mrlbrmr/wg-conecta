...
import { RESOURCES, SIDEBAR_EXTRA } from "@/lib/admin-resources";
...
function AdminLayout() {
  ...
  // Group by section
  const sections = new Map<string, { key: string; label: string; icon: React.ComponentType<{ className?: string }>; }[]>();
  const dash = { key: "", label: "Dashboard", icon: LayoutDashboard, section: "Início" };
  sections.set("Início", [{ key: dash.key, label: dash.label, icon: dash.icon }]);
  RESOURCES.forEach(r => {
  ...
  SIDEBAR_EXTRA.forEach(x => {
    ...
    arr.push({ key: x.key, label: x.label, icon: x.icon });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="flex">
        <aside className={`${open ? "block" : "hidden"} lg:block fixed lg:static inset-0 top-[57px] lg:top-0 z-30 w-full lg:w-72 bg-sidebar border-r border-sidebar-border overflow-y-auto lg:min-h-screen`}>
          <div className="hidden lg:flex items-center gap-2 px-6 py-5 border-b border-sidebar-border">
            ...
            <div>
              ...
              <div className="text-[11px] font-semibold text-muted-foreground">Gente &amp; Gestão</div>
            </div>
          </div>
              ...
              <div key={sec}>
                ...
                <div className="space-y-0.5">
                  {items.map(it => {
                    ...
                    if (isResource) {
                      return (
                        <Link key={it.key} to="/admin/recurso/$key" params={{ key: it.key }} className={className}>
          ...
          <div className="p-4 border-t border-sidebar-border mt-4">
            ...
            <button onClick={signOut} className="w-full inline-flex items-center gap-2 rounded-lg border border-input px-3 py-2 text-sm font-bold hover:bg-secondary">
