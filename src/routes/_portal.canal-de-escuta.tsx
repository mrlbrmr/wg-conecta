...
import { ESCUTA_CATEGORIES, escutaSchema } from "@/lib/channel-defs";
...
/**
 * Canal de Escuta: relato sempre sem autor. Nome e contato só vão se a pessoa marcar que quer
 * se identificar — e, mesmo assim, ficam só no texto do relato (ver `channel.functions.ts`).
 */
function CanalDeEscutaPage() {
  ...
  const [receipt, setReceipt] = useState<{ protocol: string; key: string | null } | null>(null);
  ...
  const mutation = useMutation({
    mutationFn: (payload: Record<string, unknown>) =>
      submit({ data: { channel: "escuta", identified: false, payload } }),
    onSuccess: (res) => {
      setReceipt(res);
      setForm(EMPTY);
      setIdentify(false);
  ...
  return (
    <aside className="flex flex-col gap-4 lg:sticky lg:top-24">
      ...
      <p className="text-[13px] leading-[1.6] text-muted-foreground">
        Em risco imediato, não espere: procure seu gestor ou o G&amp;G, ou ligue{" "}
        <strong>190</strong>.
      </p>
    </aside>
  );
