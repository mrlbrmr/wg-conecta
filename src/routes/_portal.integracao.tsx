  ...
  return (
    <>
      <PageHeader
        eyebrow="Boas-vindas"
        title="Integração"
        description="Materiais para novos colaboradores e para consulta geral."
      />
      {items.length === 0 ? (
        <EmptyState title="Nenhum material publicado ainda." />
      ) : (
        <div className="grid gap-4 md:grid-cols-2">
          {items.map((m) => (
            <article key={m.id} className="card-soft overflow-hidden">
              {m.image_url && (
                <img src={fileUrl(m.image_url) ?? ""} alt="" className="h-40 w-full object-cover" />
              )}
              <div className="p-6">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  {m.attachment_url && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-1.5 rounded-full border border-input bg-surface px-4 py-2 text-sm font-bold hover:bg-secondary"
                    >
