...
import { PRIVACY_NOTE, isFormSlug } from "@/lib/form-defs";
...
function FormCard({ form: f }: { form: FormRow }) {
  ...
  const body = (
    <>
      <div className="flex items-start justify-between gap-3">
        ...
        {f.category && (
          <span className="text-right text-[10px] font-extrabold uppercase tracking-[0.14em] text-primary">
            {f.category}
          </span>
        )}
      </div>
      ...
      <div className="mt-auto flex items-center justify-between gap-3 border-t border-border pt-4">
        ...
        <span className="shrink-0 text-xs font-extrabold uppercase tracking-[0.12em] text-ink">
          Preencher ↗
        </span>
      </div>
    </>
  ...
  // Formulário interno abre no portal. Os que ainda vivem fora (link do catálogo)
  // continuam abrindo em outra aba, como sempre abriram.
  if (f.slug && isFormSlug(f.slug)) {
    return (
      <PaperCard hover asChild className="flex flex-col p-6 hover:text-ink">
        <Link to="/formularios/$slug" params={{ slug: f.slug }}>
          {body}
        </Link>
      </PaperCard>
    );
